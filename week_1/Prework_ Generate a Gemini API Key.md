...
To keep your API key secure and separate from your code, save it to an environment variable called \`GEMINI\_API\_KEY\`.

\<details\>

\<summary\>On Mac:\</summary\>

\- Open your \`.bash\_profile\` or \`.zshrc\` file: \`open \~/.bash\_profile\` or \`open \~/.zshrc\`

\- Add the line: \`export GEMINI\_API\_KEY={your key}\`

\- Save and close the file, then run \`source \~/.bash\_profile\` (or \`source \~/.zshrc\`)
...
\- Set the variable name as \`GEMINI\_API\_KEY\` and paste your key as the value
